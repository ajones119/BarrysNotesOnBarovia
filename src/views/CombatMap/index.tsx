...
import ThreeMap from "@views/CombatMap/components/Map/ThreeMap";
...
const CombatMap = ({
  isPlayer = false,
  combatIdOverride = "",
  defaultMapPosition = { x: 0, y: 0 },
  setMapDefaultPosition = (_position: any) => { },
}) => {
  ...
  const { combat, isLoading, isRefetching } = useCombat(
    combatId || combatIdOverride,
  );
  const {
  ...
  useDeepCompareEffectNoCheck(() => {
    if (!isLoading && !isRefetching) {
      const tokensFromCombat = combatCharacterArray?.map(
        (character, index) => ({
  ...
  function handleDragEnd(ev: any) {
    ...
    let extraToken = extraTokens.find(
      (x: DroppableToken) => x.id === ev.active.id,
    );
    if (token) {
      token["data"]["position"] = {
      ...
      const updatedCharacterIndex = newCombat?.combatCharacterArray?.findIndex(
        (character, index) => `${index}-${character.name}` === token?.id,
      );
      newCombat.combatCharacterArray[updatedCharacterIndex] = {
        ...newCombat?.combatCharacterArray[updatedCharacterIndex],
        position: { x: token.data.position.x, y: token.data.position.y },
      };
      update({ ...newCombat });
      ...
      const updatedTokenIndex =
        map?.extraTokens?.findIndex((token) => token.id === extraToken?.id) ||
        0;
